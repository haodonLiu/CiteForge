...
impl Agent for ResearcherAgent {
    ...
    async fn run(
        ...
        tracing::info!(
            "ResearcherAgent: starting research for topic '{}'",
            input.topic
        );
        ...
        for pdf_path in &input.pdf_paths {
            ...
            let parsed = ctx
                .run_with_timeout(async { self.container.pdf_parser.parse(&bytes).await })
                .await
            ...
            for page in &parsed.pages {
                ...
                self.container
                    .vector_store
                    .add(&chunk_id, embedding, parsed.metadata.clone())
                    .await
            ...
            let search_results = ctx
                .run_with_timeout(async { self.container.search.search(&input.topic, 10).await })
                .await
                .map_err(|_| AgentError::Timeout)?
        ...
        let unique_entries: Vec<LiteratureEntry> = all_entries
            .into_iter()
            .filter(|e| {
