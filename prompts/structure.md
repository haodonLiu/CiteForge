# PCA-Lite：面向论文综述的多 Agent 协作框架

## 一、定位与目标

PCA-Lite 是一个面向论文综述撰写的多 Agent 协作框架，CLI 先行，Web UI 后续跟进。核心目标：
...
预处理层是 PCA-Lite 处理大规模文献的基础设施。它是纯工程层，不经过 LLM，负责把"原始文件"转化为"Agent 可消费的结构化数据"。
