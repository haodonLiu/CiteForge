...
from citeforge.core.consts import DIR_RAW_PDFS, DIR_PREPROCESSED, FILE_DRAFT, FILE_LITERATURE_POOL, FILE_STATE, FILE_THEMES
from citeforge.core.exceptions import PipelineError
from citeforge.core.models import LiteratureEntry
from citeforge.llm.providers.openai import OpenAIProvider
from citeforge.llm.providers.anthropic import AnthropicProvider
...
class ExecutionRunner:
    ...
    def _build_search_provider(self, search_cfg: dict):
        ...
        if not api_key:
            return None
        from citeforge.search.semantic_scholar import SemanticScholarSearch
    ...
    async def _run_async(
        ...
        from citeforge.agents.researcher import ResearcherAgent
        from citeforge.agents.analyst import AnalystAgent
        from citeforge.agents.writer import WriterAgent
    ...
    async def _execute_step(
        ...
        if step.agent == AgentType.ORCHESTRATOR:
            ...
            from citeforge.ingestion.parser import extract_all_pdfs
...
def run_execution(topic: str, files: list[str], workspace_dir: Path, callback=None) -> None:
    ...
    if config_path.exists():
        ...
        from citeforge.core.models import LLMConfig, EmbeddingConfig, Config
        cfg = Config(
