...
impl LlmRegistry {
    ...
    pub fn create_chat(&self, config: &ProviderConfig) -> Result<Arc<dyn ChatProvider>, CiteForgeError> {
        let factory = self.chat_factories.get(&config.provider_type)
            .ok_or_else(|| CiteForgeError::Application(format!("unknown chat provider: {}", config.provider_type)))?;

        factory.create(config)
    }

    pub fn create_embed(&self, config: &ProviderConfig) -> Result<Arc<dyn EmbedProvider>, CiteForgeError> {
        let factory = self.embed_factories.get(&config.provider_type)
            .ok_or_else(|| CiteForgeError::Application(format!("unknown embed provider: {}", config.provider_type)))?;
