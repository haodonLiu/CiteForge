...
use citeforge_core::ports::{SearchResult, VectorStore};
...
impl ChromaStore {
    ...
    pub async fn ensure_collection(&self) -> Result<(), ChromaError> {
        ...
        if !resp.status().is_success() {
            return Err(ChromaError::Connection(format!(
    ...
    pub async fn add_batch(
        &self,
        items: Vec<(String, Vec<f32>, DocumentMetadata)>,
    ) -> Result<(), ChromaError> {
    ...
    async fn add_chunk(
        &self,
        items: &[(String, Vec<f32>, DocumentMetadata)],
    ) -> Result<(), ChromaError> {
        ...
        let metadatas: Vec<serde_json::Value> = items
            .iter()
            .map(|(_, _, m)| {
                json!({
...
impl VectorStore for ChromaStore {
    ...
    async fn add(
        ...
        self.add_batch(vec![(id.to_string(), embedding, metadata)])
            .await
    }
    ...
    async fn search(
        ...
        let ids = result["ids"][0]
            .as_array()
            .ok_or_else(|| ChromaError::Parse("no ids".to_string()))?;
        let distances = result["distances"][0]
            .as_array()
            .ok_or_else(|| ChromaError::Parse("no distances".to_string()))?;
        let metadatas = result["metadatas"][0]
            .as_array()
            .ok_or_else(|| ChromaError::Parse("no metadatas".to_string()))?;
